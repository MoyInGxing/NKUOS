...
exit(int error_code) {
  ...
  8000dc:	52050513          	addi	a0,a0,1312 # 8005f8 <main+0xde>
...
umain(void) {
  ...
  8000f4:	e406                	sd	ra,8(sp)
    int ret = main();
  8000f6:	424000ef          	jal	ra,80051a <main>
    exit(ret);
...
printnum(void (*putch)(int, void*), void *putdat,
  ...
  80013c:	4d878793          	addi	a5,a5,1240 # 800610 <main+0xf6>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  ...
  800198:	4b0a8a93          	addi	s5,s5,1200 # 800644 <main+0x12a>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
  ...
  8001a0:	6c4b8b93          	addi	s7,s7,1732 # 800860 <error_string>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  ...
  8003f6:	24e60613          	addi	a2,a2,590 # 800640 <main+0x126>
  ...
  800408:	22c60613          	addi	a2,a2,556 # 800630 <main+0x116>
  ...
  80041a:	21240413          	addi	s0,s0,530 # 800628 <main+0x10e>
  ...
  8004a4:	18840413          	addi	s0,s0,392 # 800628 <main+0x10e>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  ...
  8004ba:	173a0a13          	addi	s4,s4,371 # 800629 <main+0x10f>
...
memset(void *s, char c, size_t n) {
    ...
    char *p = s;
    while (n -- > 0) {
  800508:	ca01                	beqz	a2,800518 <memset+0x10>
  80050a:	962a                	add	a2,a2,a0
    char *p = s;
  80050c:	87aa                	mv	a5,a0
        *p ++ = c;
  80050e:	0785                	addi	a5,a5,1
  800510:	feb78fa3          	sb	a1,-1(a5)
    while (n -- > 0) {
  800514:	fec79de3          	bne	a5,a2,80050e <memset+0x6>
    }
    return s;
...
int
main(void) {
  80051a:	1141                	addi	sp,sp,-16
    // 1. 分配并初始化测试内存
    memset(buffer, 'A', TEST_SIZE);
  80051c:	6611                	lui	a2,0x4
  80051e:	04100593          	li	a1,65
  800522:	00001517          	auipc	a0,0x1
  800526:	ade50513          	addi	a0,a0,-1314 # 801000 <buffer>
main(void) {
  80052a:	e406                	sd	ra,8(sp)
  80052c:	e022                	sd	s0,0(sp)
    memset(buffer, 'A', TEST_SIZE);
  80052e:	fdbff0ef          	jal	ra,800508 <memset>
    cprintf("Parent %04x: Initialize buffer with 'A'\n", getpid());
  800532:	bbfff0ef          	jal	ra,8000f0 <getpid>
  800536:	85aa                	mv	a1,a0
  800538:	00000517          	auipc	a0,0x0
  80053c:	3f050513          	addi	a0,a0,1008 # 800928 <error_string+0xc8>
  800540:	b01ff0ef          	jal	ra,800040 <cprintf>
    
    // 2. 创建子进程
    int pid = fork();
  800544:	ba3ff0ef          	jal	ra,8000e6 <fork>
    if (pid == 0) {
  800548:	c521                	beqz	a0,800590 <main+0x76>
        ...
        // 父进程
        yield();  // 让子进程先运行
  80054a:	ba5ff0ef          	jal	ra,8000ee <yield>
        
        // 检查原始内存内容
        cprintf("Parent %04x: My first byte is still: %c\n", 
  80054e:	ba3ff0ef          	jal	ra,8000f0 <getpid>
  800552:	85aa                	mv	a1,a0
  800554:	00001617          	auipc	a2,0x1
  800558:	aac64603          	lbu	a2,-1364(a2) # 801000 <buffer>
  80055c:	00000517          	auipc	a0,0x0
  800560:	48450513          	addi	a0,a0,1156 # 8009e0 <error_string+0x180>
  800564:	addff0ef          	jal	ra,800040 <cprintf>
                getpid(), buffer[0]);
        cprintf("Parent %04x: Page 2 first byte is still: %c\n", 
  800568:	b89ff0ef          	jal	ra,8000f0 <getpid>
  80056c:	85aa                	mv	a1,a0
  80056e:	00002617          	auipc	a2,0x2
  800572:	a9264603          	lbu	a2,-1390(a2) # 802000 <buffer+0x1000>
  800576:	00000517          	auipc	a0,0x0
  80057a:	49a50513          	addi	a0,a0,1178 # 800a10 <error_string+0x1b0>
  80057e:	ac3ff0ef          	jal	ra,800040 <cprintf>
                getpid(), buffer[PAGE_SIZE]);
        
        wait();
  800582:	b67ff0ef          	jal	ra,8000e8 <wait>
    }
    return 0;
  800586:	60a2                	ld	ra,8(sp)
  800588:	6402                	ld	s0,0(sp)
  80058a:	4501                	li	a0,0
  80058c:	0141                	addi	sp,sp,16
  80058e:	8082                	ret
        cprintf("Child  %04x: Original first byte: %c\n", getpid(), buffer[0]);
  800590:	b61ff0ef          	jal	ra,8000f0 <getpid>
  800594:	00001417          	auipc	s0,0x1
  800598:	a6c40413          	addi	s0,s0,-1428 # 801000 <buffer>
  80059c:	00044603          	lbu	a2,0(s0)
  8005a0:	85aa                	mv	a1,a0
  8005a2:	00000517          	auipc	a0,0x0
  8005a6:	3b650513          	addi	a0,a0,950 # 800958 <error_string+0xf8>
  8005aa:	a97ff0ef          	jal	ra,800040 <cprintf>
        buffer[0] = 'B';
  8005ae:	04200793          	li	a5,66
  8005b2:	00f40023          	sb	a5,0(s0)
        cprintf("Child  %04x: Modified first byte to: %c\n", getpid(), buffer[0]);
  8005b6:	b3bff0ef          	jal	ra,8000f0 <getpid>
  8005ba:	00044603          	lbu	a2,0(s0)
  8005be:	85aa                	mv	a1,a0
  8005c0:	00000517          	auipc	a0,0x0
  8005c4:	3c050513          	addi	a0,a0,960 # 800980 <error_string+0x120>
  8005c8:	a79ff0ef          	jal	ra,800040 <cprintf>
        buffer[PAGE_SIZE] = 'C';
  8005cc:	04300793          	li	a5,67
  8005d0:	00002417          	auipc	s0,0x2
  8005d4:	a3040413          	addi	s0,s0,-1488 # 802000 <buffer+0x1000>
  8005d8:	00f40023          	sb	a5,0(s0)
        cprintf("Child  %04x: Modified page 2 first byte to: %c\n", 
  8005dc:	b15ff0ef          	jal	ra,8000f0 <getpid>
  8005e0:	00044603          	lbu	a2,0(s0)
  8005e4:	85aa                	mv	a1,a0
  8005e6:	00000517          	auipc	a0,0x0
  8005ea:	3ca50513          	addi	a0,a0,970 # 8009b0 <error_string+0x150>
  8005ee:	a53ff0ef          	jal	ra,800040 <cprintf>
        exit(0);
  8005f2:	4501                	li	a0,0
  8005f4:	addff0ef          	jal	ra,8000d0 <exit>
